...
@Slf4j
@RequestMapping(value = "<#if package.ModuleName??>/${package.ModuleName}</#if>/<#if controllerMappingHyphenStyle??>${controllerMappingHyphen}<#else>${table.entityPath}</#if>", produces = {"application/json;charset=UTF-8"})
<#if superControllerClass??>public class ${table.controllerName} extends ${superControllerClass}{<#else>public class ${table.controllerName} {</#if>
    ...
    public Boolean updates(String jsonList) {
        List< ${entity}> ${entity?uncap_first}List = parseJson2Obj(jsonList,  ${entity}.class);
        List<Long> ids = ${entity?uncap_first}List.stream().filter(u->Objects.nonNull(u.getId())).map(${entity}::getId).collect(Collectors.toList());
        idsCheck(ids);
   ...
   private <T> void idsCheck(List<T> ids) {
       ...
       int dbCount = ${(table.serviceName?substring(0))?uncap_first}.count(new QueryWrapper<${entity}>().lambda().in(${entity}::getId, ids));
       if (dbCount != ids.size()) {
